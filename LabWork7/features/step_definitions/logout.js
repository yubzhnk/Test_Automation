...
console.log("9. Click 'Logout' button");
console.log("10. Verify that user is navigated to login page");
...
Then('User verify that {string} is visible', async function (text) {
    await driver.wait(until.elementLocated(By.xpath(`//*[contains(text(), '${text}')]`)), 25000);
});
...
Then('User verify that user is navigated to login page', async function () {
    const expectedTitle = 'Automation Exercise - Signup / Login';
    const actualTitle = await driver.getTitle();
    assert.strictEqual(actualTitle, expectedTitle, 'User is not navigated to login page');
});
